...
export default function ProductPage() {
  return (
    <div className="pt-32 pb-20 min-h-screen bg-[#050505] text-[#f7f7f5]">
      <div className="container max-w-7xl mx-auto px-4 space-y-12">
        <div className="text-center space-y-4 max-w-3xl mx-auto">
          <span className="text-xs font-semibold tracking-widest text-[#858583] uppercase">Digital Products</span>
          <h1 className="text-4xl sm:text-6xl font-bold">Produk Digital Hammad Studio</h1>
          <p className="text-base text-[#c3c3c0]">
          ...
          {products.map((prod, idx) => (
            <div key={idx} className="p-6 rounded-3xl bg-[#111111] border border-white/10 space-y-4 hover:border-white/30 transition-colors">
              <div className="relative w-full h-64 rounded-2xl overflow-hidden bg-zinc-900 border border-white/10">
                <Image src={prod.image} alt={prod.title} fill className="object-cover grayscale hover:grayscale-0 transition-all duration-500" />
              </div>
              ...
              <p className="text-sm text-[#c3c3c0]">{prod.description}</p>
              <Link
