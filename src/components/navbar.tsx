  ...
  return (
    <header className="pointer-events-none fixed inset-x-0 top-0 z-50 px-4 pt-4 sm:px-5">
      ...
      >
        ...
        <a href={projectUrl} target="_blank" rel="noreferrer" className="acid-button group hidden items-center gap-2 rounded-full px-4 py-2.5 text-[0.7rem] font-semibold lg:inline-flex">Let&apos;s talk <ArrowUpRightIcon className="button-arrow h-3.5 w-3.5" /></a>
        <button type="button" onClick={() => setOpen((value) => !value)} className="flex h-9 w-9 items-center justify-center rounded-full border border-white/12 text-white lg:hidden" aria-expanded={open} aria-controls="mobile-navigation" aria-label={open ? "Close navigation" : "Open navigation"}>
      ...
      <AnimatePresence>
        {open && (
          <motion.div id="mobile-navigation" initial={reduceMotion ? false : { opacity: 0, y: -12, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -8, scale: 0.98 }} transition={{ duration: 0.25 }} className="pointer-events-auto mx-auto mt-2 max-w-[1240px] overflow-hidden rounded-[24px] border border-white/10 bg-[#0a0a09]/96 p-5 shadow-2xl backdrop-blur-2xl lg:hidden">
            ...
            <a href={projectUrl} target="_blank" rel="noreferrer" className="acid-button group mt-5 flex w-full items-center justify-center gap-2 rounded-full px-4 py-3 text-sm font-semibold">Start a project <ArrowUpRightIcon className="button-arrow h-4 w-4" /></a>
          </motion.div>
        )}
      </AnimatePresence>
    </header>
