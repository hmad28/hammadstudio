        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {copy.items.map((item, index) => (
            <MotionReveal key={item.title} delay={index * 0.08}>
              <Link href="/pricing" className="pricing-card group flex min-h-[310px] flex-col justify-between rounded-[8px] border border-black/12 bg-[#e8e5dc] p-6 transition-all duration-500 hover:-translate-y-2 hover:bg-[#0a0a09] hover:text-white sm:min-h-[350px] sm:p-7">
                ...
                <div>
                  ...
                  <div className="mt-8 border-t border-current/15 pt-5"><span className="text-[0.65rem] opacity-45">{copy.starting[locale]}</span><p className="mt-1 text-[2.15rem] font-[500] leading-none tracking-[-0.05em]">{item.price[locale]}</p></div>
                </div>
              </Link>
            </MotionReveal>
          ))}
