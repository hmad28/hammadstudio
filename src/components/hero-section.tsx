    ...
    <section ref={sectionRef} id="hero" className="hero-surface relative min-h-screen overflow-hidden pb-10 pt-32 text-white sm:pt-36 lg:pt-40">
      ...
      <div className="site-container relative z-10">
        <div className="flex items-center justify-between border-b border-white/10 pb-4">
          <span className="label-mono text-white/45">Independent digital studio</span>
          <span className="hidden items-center gap-2 text-[0.7rem] text-white/55 sm:flex"><span className="availability-dot" /> Available for Q4 / 2026</span>
        </div>
        ...
        <div className="grid gap-8 pb-12 pt-10 lg:grid-cols-12 lg:items-end lg:pb-16 lg:pt-12">
          <h1 className="text-[clamp(4rem,9.7vw,9rem)] font-[540] leading-[0.79] tracking-[-0.072em] text-[#f6f4ee] lg:col-span-9">
            <span className="hero-line"><motion.span initial={reduceMotion ? false : { y: "110%" }} animate={{ y: 0 }} transition={{ duration: 0.9, ease: [0.16, 1, 0.3, 1] }}>Digital products</motion.span></span>
            <span className="hero-line pb-[0.08em]"><motion.span initial={reduceMotion ? false : { y: "110%" }} animate={{ y: 0 }} transition={{ duration: 0.9, delay: 0.08, ease: [0.16, 1, 0.3, 1] }}><span className="font-[family-name:var(--font-instrument-serif)] font-normal italic tracking-[-0.045em] text-[#cfef57]">made</span> to matter.</motion.span></span>
          </h1>
          ...
          <motion.div className="lg:col-span-3 lg:pb-2" initial={reduceMotion ? false : { opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.75, delay: 0.32, ease: [0.16, 1, 0.3, 1] }}>
            <p className="max-w-[320px] text-sm leading-[1.6] text-white/58 sm:text-base lg:ml-auto">We design and engineer websites, commerce, and software that move businesses forward.</p>
            <div className="mt-6 flex flex-wrap gap-3 lg:justify-end">
              <a href={projectUrl} target="_blank" rel="noreferrer" className="acid-button group inline-flex items-center gap-2 rounded-full px-5 py-3 text-xs font-semibold">Start a project <ArrowUpRightIcon className="button-arrow h-3.5 w-3.5" /></a>
              <Link href="#work" className="inline-flex items-center rounded-full border border-white/16 px-5 py-3 text-xs font-medium text-white/80 transition-colors hover:bg-white hover:text-black">Explore work</Link>
            </div>
          </motion.div>
        </div>
        ...
        <motion.div className="hero-stage relative mx-auto h-[330px] max-w-[1180px] sm:h-[490px] lg:h-[620px]" style={{ y: showcaseY }} initial={reduceMotion ? false : { opacity: 0, y: 70, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 1.05, delay: 0.38, ease: [0.16, 1, 0.3, 1] }}>
          ...
          <div className="group absolute left-1/2 top-0 z-10 h-[82%] w-[82%] -translate-x-1/2 overflow-hidden rounded-[10px] border border-white/15 bg-[#0d0d0d] shadow-[0_45px_120px_rgba(0,0,0,.7)] sm:w-[70%]">
            ...
            <div className="absolute inset-x-0 bottom-0 flex items-end justify-between bg-gradient-to-t from-black/80 to-transparent p-4 pt-16 sm:p-6 sm:pt-24"><div><p className="label-mono text-white/45">Featured project / 2026</p><p className="mt-2 text-sm font-medium sm:text-lg">Saudi Education Expo</p></div><span className="flex h-9 w-9 items-center justify-center rounded-full bg-[#cfef57] text-black sm:h-11 sm:w-11"><ArrowUpRightIcon className="h-4 w-4" /></span></div>
          </div>
