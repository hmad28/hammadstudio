...
export function WhyItMattersSection() {
  ...
  return (
    <section
      id="why-it-matters"
      className="why-surface-v2 scroll-mt-20 py-20 text-[#17181d] sm:py-28 lg:py-32"
    >
      <div className="site-container">
        <MotionReveal className="surface-card relative overflow-hidden rounded-2xl bg-white/80 border border-[#dde0e7] shadow-xl lg:grid-cols-12 lg:grid">
          {/* Ambient Glow */}
          <div
            className="pointer-events-none absolute -left-20 -top-20 h-72 w-72 rounded-full bg-[radial-gradient(circle_at_center,rgba(128,87,255,0.18)_0%,transparent_70%)] blur-2xl"
            aria-hidden="true"
          />
          ...
          <div className="relative min-h-[380px] overflow-hidden bg-[#070a12] sm:min-h-[500px] lg:col-span-6">
            <Image
              ...
              alt="Solivate Studio website homepage"
              fill
          ...
          <div className="flex flex-col justify-center p-8 sm:p-12 lg:col-span-6 lg:p-14">
            ...
            <div className="mt-8 grid gap-3 sm:grid-cols-2">
              {copy.items.map((item, index) => {
                const IconComp =
                  iconMap[item.icon as keyof typeof iconMap] || ShieldCheckIcon;
                const iconStyle =
                  benefitIconStyles[index % benefitIconStyles.length];

                return (
                  <div
                    key={item.title}
                    className="rounded-xl border border-black/8 bg-white/90 p-4 shadow-sm backdrop-blur-md"
                  >
                    <div className="flex items-center gap-2.5">
                      <span
                        className={`flex h-7 w-7 items-center justify-center rounded-md ${iconStyle}`}
                      >
